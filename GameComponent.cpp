...
void TriangleGameComponent::UP_DOWN(Keys key,float velocity)
{
}
...
void TriangleGameComponent::CheckPlayersCollisions(GameComponent* player_1, GameComponent* player_2)
{
}

void TriangleGameComponent::Update(float deltaTime,float y)
{
	if ((pPoints[0].y + pPoints[4].y) / 2 < y && pPoints[0].y <= 1.0f)
	{
	...
	else if ((pPoints[0].y + pPoints[4].y) / 2 > y && pPoints[0].y >= -1.0f)
	{
		pPoints[0].y -= deltaTime * speed;
		pPoints[2].y -= deltaTime * speed;
		pPoints[4].y -= deltaTime * speed;
...
DirectX::XMFLOAT4* TriangleGameComponent::GetPoints()
{
	return pPoints.data();
}
